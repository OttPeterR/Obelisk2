...
public class Wireframe
{
	...
	public void rotate(double rot)
	{
		for (int i = 0; i < numCoords; i++)
		{
			double Xo = coords[i].x; // temp X location for use in y
										// coords modification
			coords[i].x = xCenter + ((Xo - xCenter) * Math.cos(rot) - (coords[i].y - yCenter) * Math.sin(rot));
			coords[i].y = yCenter + ((Xo - xCenter) * Math.sin(rot) + (coords[i].y - yCenter) * Math.cos(rot));
		}
	}
	...
	public void render(Graphics2D g, double origX, double origY, Color c)
	{
		...
		for (int i = 0; i < numCoords - 1; i++)
		{
			g.drawLine((int) (coords[i].x - origX), (int) (coords[i].y - origY), (int) (coords[i + 1].x - origX), (int) (coords[i + 1].y - origY));
			g.fillOval((int) (coords[i].x - origX - 2), (int) (coords[i].y - origY - 2), 4, 4);
		}
		// the below line is needed to close the shape
		g.fillOval((int) (coords[numCoords - 1].x - origX - 2), (int) (coords[numCoords - 1].y - origY - 2), 4, 4);

		g.drawLine((int) (coords[0].x - origX), (int) (coords[0].y - origY), (int) (coords[numCoords - 1].x - origX),
				(int) (coords[numCoords - 1].y - origY));
		g.drawOval((int) (xCenter - 5 - origX), (int) (yCenter - 5 - origY), 10, 10);
	}
	...
	public ArrayList<Line2D.Double> getLineWireFrame()
	{
		...
		for (int i = 0; i < numCoords - 1; i++)
		{
			ret.add(new Line2D.Double(coords[i].x, coords[i].y, coords[i + 1].x, coords[i + 1].y));
		}
		ret.add(new Line2D.Double(coords[numCoords - 1].x, coords[numCoords - 1].y, coords[0].x, coords[0].y));
